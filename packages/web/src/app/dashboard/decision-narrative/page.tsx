    ...
    <button
      ...
      className={`w-full text-left rounded-xl border p-4 transition ${
        isActive
          ...
          : "border-neutral-800 bg-neutral-900/70 hover:border-emerald-900/60"
      }`}
    >
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-2">
          <p className="font-medium text-white">{firstInsight?.title ?? "Decision trace"}</p>
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className="rounded-full px-2 py-1 border border-neutral-700 text-neutral-300">
              {trace.trace_type}
            </span>
            <span className="rounded-full px-2 py-1 border border-neutral-700 text-neutral-300 flex items-center gap-1">
              {ruleCount} rule checks
            </span>
            {isRecent ? <span className="rounded-full px-2 py-1 border border-emerald-900 text-emerald-300">Fresh</span> : null}
          </div>
        </div>

        <ChevronRight className="w-4 h-4 text-neutral-500 mt-1 shrink-0" />
      </div>
    ...
    <div className="min-h-screen bg-neutral-950">
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 lg:px-8 py-8">
        <motion.div
          ...
          className="mb-6"
        >
          <p className="text-sm uppercase tracking-[0.25em] text-emerald-400">Narrative AI Layer</p>
          <h1 className="mt-2 font-serif text-3xl text-white">Decision Narrative</h1>
        ...
        >
          <div className="grid lg:grid-cols-3 gap-6">
            <section className="lg:col-span-1 space-y-3">
              <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-4 flex flex-wrap gap-2">
                {[
                  { label: "All", value: "all" },
              ...
              {filtered.length === 0 ? (
                <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-5 text-neutral-400 text-sm">
              ...
              {!active ? (
                ...
                <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-6 space-y-6">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-sm text-neutral-400">Trace details</p>
                    ...
                    <span className="text-xs rounded-full border border-neutral-700 px-2 py-1 text-neutral-400">
                  ...
                  <div className="rounded-lg border border-neutral-800 bg-neutral-950 p-4">
                    ...
                    <div className="mt-3 space-y-3">
                      {getDeterministic(active).rulesApplied.length === 0 ? (
                        <p className="text-sm text-neutral-400">No deterministic rules were recorded.</p>
                      ) : (
                        getDeterministic(active).rulesApplied.map((rule, index) => (
                          ...
                          >
                            <div className="flex items-center justify-between text-sm">
                              <p className="text-neutral-100">{rule.name}</p>
                              <span
                            ...
                            {rule.message ? <p className="mt-1 text-xs text-neutral-400">{rule.message}</p> : null}
                          </article>
                        ))
                      )}
                    </div>
                  ...
                  <div className="rounded-lg border border-neutral-800 bg-neutral-950 p-4">
                    <h3 className="text-sm font-medium text-white flex items-center gap-2">
                      <CircleEllipsis className="w-4 h-4 text-emerald-400" />
                      Input snapshot
                    </h3>
                    <pre className="mt-3 text-xs text-neutral-300 overflow-auto whitespace-pre-wrap">
