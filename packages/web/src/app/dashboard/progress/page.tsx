...
const ProgressNetWorthChart = dynamic(
  () => import("@/components/dashboard/ProgressNetWorthChart").then(m => m.ProgressNetWorthChart),
...
import { ProgressLoadingSkeleton } from "@/components/shared/LoadingSkeleton";
...
function toneToStyle(level: RiskBand) {
  if (level === "good") {
    return {
      badge: "text-emerald-700 dark:text-emerald-200 bg-emerald-50 dark:bg-emerald-900/25 border-emerald-200 dark:border-emerald-700",
      bar: "bg-emerald-500",
  ...
  if (level === "watch") {
    return {
      badge: "text-amber-700 dark:text-amber-200 bg-amber-50 dark:bg-amber-900/25 border-amber-200 dark:border-amber-700",
      bar: "bg-amber-500",
      value: "text-amber-700 dark:text-amber-200",
    };
  }
  return {
    badge: "text-rose-700 dark:text-rose-200 bg-rose-50 dark:bg-rose-900/25 border-rose-200 dark:border-rose-700",
    bar: "bg-rose-500",
...
export default function ProgressPage() {
  ...
  const rangeLabel = range === "90d" ? "90D" : range === "1y" ? "1Y" : "ALL";
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] dark:bg-slate-950 transition-colors duration-500">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 py-8">
        <Breadcrumbs items={[{ label: "Dashboard", href: "/dashboard" }, { label: "Progress" }]} />
        <motion.div
        ...
        >
          ...
          <h1 className="mt-2 font-serif text-3xl text-slate-900 dark:text-white">Founder Progress</h1>
        ...
        >
          <section className="grid lg:grid-cols-4 gap-4 mb-6">
            <article className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
              <p className="text-sm text-slate-600 dark:text-slate-400">Net worth change ({rangeLabel})</p>
              <p className="mt-2 text-2xl font-semibold text-slate-900 dark:text-white">{formatCurrency(derived.change)}</p>
              <p
            ...
            <article className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
              <p className="text-sm text-slate-600 dark:text-slate-400">Runway</p>
              <p className="mt-2 text-2xl font-semibold text-slate-900 dark:text-white">{derived.runway.toFixed(1)} mo</p>
              <p className="mt-2 text-xs text-slate-600 dark:text-slate-400">Against {formatCurrency(derived.monthlyBurn)} monthly burn.</p>
            </article>

            <article className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
              <p className="text-sm text-slate-600 dark:text-slate-400">Debt pressure</p>
              <p className="mt-2 text-2xl font-semibold text-slate-900 dark:text-white">{formatPercent(derived.debtCoverage, 1)}</p>
              <p className="mt-2 text-xs text-slate-600 dark:text-slate-400">Debt versus annual income coverage.</p>
            </article>

            <article className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
              <p className="text-sm text-slate-600 dark:text-slate-400">Cash reserve ratio</p>
              <p className="mt-2 text-2xl font-semibold text-slate-900 dark:text-white">{formatPercent(derived.reserveRatio, 1)}</p>
              <p className="mt-2 text-xs text-slate-600 dark:text-slate-400">Cash compared to annual income target.</p>
            </article>
          </section>

          <ProgressNetWorthChart
            timeline={timeline}
            range={range}
            onRangeChange={setRange}
            minNetWorth={derived.minNetWorth}
            maxNetWorth={derived.maxNetWorth}
          />
            ...
            {derived.milestones.map((milestone) => {
              ...
              return (
                  ...
                  <div className="flex items-center justify-between">
                    <p className="text-sm text-slate-900 dark:text-white">{milestone.label}</p>
          ...
          <section className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-lg text-slate-900 dark:text-white font-medium">Next actions</h2>
            ...
            <div className="grid md:grid-cols-3 gap-3">
              <Link
                ...
                className="rounded-lg border border-slate-300 dark:border-slate-700 p-3 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800"
              >
              ...
              <Link
                ...
                className="rounded-lg border border-slate-300 dark:border-slate-700 p-3 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center gap-2"
              >
