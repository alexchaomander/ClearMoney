...
const VALUATIONS = [
  { program: "Chase Ultimate Rewards", value: "2.05 cpp", delta: "+0.05" },
  { program: "Amex Membership Rewards", value: "1.80 cpp", delta: "-0.02" },
  { program: "Bilt Rewards", value: "2.10 cpp", delta: "0.00" },
  { program: "Capital One Venture", value: "1.75 cpp", delta: "+0.01" }
];

export function BlogPageClient({ posts }: { posts: PostMeta[] }) {
  return (
    <div className="min-h-screen bg-[#fafafa] text-slate-900 selection:bg-emerald-500/30">
      {/* Editorial Header */}
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <span className="font-display text-xl tracking-tight">
              Clear<span className="text-emerald-600">Money</span> <span className="text-slate-400 font-normal">Intelligence</span>
            </span>
          </Link>

          <nav className="hidden md:flex items-center gap-8">
            <Link href="/dashboard" className="text-sm font-bold text-slate-600 hover:text-emerald-600 transition-colors flex items-center gap-1.5">
      ...
      <main className="max-w-7xl mx-auto px-6 lg:px-8 py-20">
        <div className="grid lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-8 space-y-20">
            <section>
              <div className="flex items-center gap-2 text-emerald-600 font-bold text-[10px] uppercase tracking-[0.2em] mb-4">
                <Newspaper className="w-3 h-3" />
                Latest Analysis
              </div>
              <h1 className="font-display text-5xl md:text-6xl text-slate-900 mb-8 leading-[1.1] tracking-tight">
                  ...
                  <Link key={post.slug} href={`/blog/${post.slug}`} className="group block">
                    <article className="grid md:grid-cols-4 gap-8">
                      ...
                      <div className="md:col-span-3">
                        <h2 className="font-display text-2xl md:text-3xl text-slate-900 group-hover:text-emerald-600 transition-colors mb-4">
                          {post.title}
                        </h2>
                        ...
                        <div className="flex items-center gap-2 text-xs font-bold text-slate-900 group-hover:gap-3 transition-all">
                          READ ANALYSIS <ArrowRight className="w-3.5 h-3.5 text-emerald-600" />
                        </div>
                      </div>
                    </article>
                  </Link>
          ...
          <div className="lg:col-span-4 space-y-12">
            ...
            <div className="p-8 rounded-3xl border border-slate-200 bg-white">
              <h3 className="font-display text-xl mb-2">Field Notes</h3>
              ...
              <form className="space-y-3">
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all"
                />
                <button className="w-full py-3 rounded-xl bg-slate-900 text-white font-bold text-sm hover:bg-slate-800 transition-all">
                  Subscribe
                </button>
              </form>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {["Credit Strategy", "Tax Harvest", "Yield Arbitrage", "ACATS Rollover", "Equity Comp"].map(t => (
                  <span key={t} className="px-3 py-1.5 rounded-full bg-slate-100 text-slate-600 text-xs font-medium border border-slate-200 hover:border-slate-300 cursor-pointer transition-colors">
                    {t}
                  </span>
                ))}
              </div>
