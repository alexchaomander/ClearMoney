...
def run_deterministic_checks(context: dict) -> dict:
    ...
    high_interest = [
        d for d in debt_accounts if (_safe_float(d.get("interest_rate")) or 0) >= 0.1
    ]
    if high_interest:
        total_balance = sum((_safe_float(d.get("balance")) or 0) for d in high_interest)
        max_rate = max(
            (_safe_float(d.get("interest_rate")) or 0) for d in high_interest
        )
        insights.append(
            {
                ...
                "data_points": {
                    "high_interest_balance": total_balance,
                    "max_rate": max_rate,
                },
            }
        )
    ...
    if annual_income and monthly_retirement is not None:
        ...
        assumptions.append(
            "Annual income or retirement contribution missing; savings rate check skipped."
        )
    ...
    if monthly_income and monthly_savings_target is not None:
        savings_rate = (
            monthly_savings_target / monthly_income if monthly_income > 0 else 0
        )
        rules_applied.append(
            {
        ...
        assumptions.append(
            "Monthly savings target missing; savings rate check skipped."
        )
    ...
    if total_investment > 0 and holdings:
        top_holding = max(
            holdings, key=lambda h: _safe_float(h.get("market_value")) or 0
        )
            ...
            insights.append(
                {
                    ...
                    "data_points": {
                        "top_holding": top_holding.get("ticker"),
    ...
    if not recent_transactions:
        assumptions.append(
            "No recent transactions available; cash flow signals may be limited."
        )
