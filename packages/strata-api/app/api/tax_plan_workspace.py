...
async def _get_plan_accessible(
    ...
    *,
    ...
    if not collaborator:
        # Keep same behavior for non-visible plans.
        raise HTTPException(status_code=404, detail="Tax plan not found")
...
async def approve_tax_plan_version(
    ...
    prev_versions = await session.execute(
        select(TaxPlanVersion).where(TaxPlanVersion.plan_id == plan.id)
    )
    for row in prev_versions.scalars().all():
        row.is_approved = row.id == version.id
