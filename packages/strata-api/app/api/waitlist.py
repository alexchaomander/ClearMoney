...
async def join_waitlist(
    ...
    try:
        ...
        result = await db.execute(
            select(WaitlistUser).where(WaitlistUser.email == data.email)
        )
...
async def get_waitlist_status(referral_code: str, db: AsyncSession = Depends(get_db)):
    """Get waitlist status by referral code."""
    result = await db.execute(
        select(WaitlistUser).where(WaitlistUser.referral_code == referral_code)
    )
